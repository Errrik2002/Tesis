...
######## EJEMPLO DE EXAMEN SEGUNDO PARCIAL#######
nu <- 1/4
beta <- 3
mu <- 1/4
rho <- 1/2
...
immpasiva <- function(t, state, parameters){
...
parameters <- c(nu, beta, mu, rho, alpha) 
state <- c(M = 0.90, S = 0.05, I= 0.05, R= 0)
times <- seq(0,20, by=1)
...
with(as.list(c(state, parameter)),{
  dP <- k1*dP*(times-t1) - k2*dP - k3*dAB*dP - k4*dM1*dP - k5*dP
  dPi <- k5&dP - k1*dPi*(t-t1) - k6*dPi*dCTL 
  dM <- k7*(M0-dM) - k8*dM*dP
  dM1 <- k8*M*dP - k9*dM1
  dDC <- k10*(DC0-dDC) - k11*dDC*dP
  dDC1 <- k11*dDC*dP - k12*dDC1
  dDCm <- k13*dDC1*(times-t2) - k14*dDCm
  dTh0 <- k15*(th0_0 - dth0) - k16*dDCm*dth0
