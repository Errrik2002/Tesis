...
mu6 <- 1*10**-6 #11
alpha5 <- 0.6 #12
alpha7 <- 0.019 #SECRECION DE IL10 #13
...
trypanosoma_29_NOV <- function(t,state,parameter){
  with(as.list(c(state,parameter)),  {
    ...
    d_M <- (nu1*((TNF**h1)/((n3**h3)+TNF**h3))*((n4**h4)/((n4**h4)+IL10**h4)))*(M-M0)+(alpha1*((TNF**h3)/((n3**h3)+TNF**h3))*(n4**h4)/((n4**h4)+IL10**h4))*TL*M+(mu2*((TNF**h3)/((n3**h3)+TNF**h3))*((n4**h4)/((n4**h4)+IL10**h4)))*M
    ...
    d_TNF <- (alpha5*((n2**h2)/((n2**h2)+IL10**h2)))*Mi-mu7*(TNF-qTNF)
...
png("images/29_NOV_2024_NOSIGNS.png")
plot(out29_NOV,col='red')
dev.off()
